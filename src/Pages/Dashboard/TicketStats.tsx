...
export default function TicketStats({ title, value, percentage, type }: TicketStatsProps) {
    ...
    return (
        <div className={`relative overflow-hidden bg-gradient-to-br ${color.grad}
            ...
            <div className={`w-11 h-11 rounded-xl ${color.iconBg} ${color.iconRing} flex items-center justify-center`}>
                {isMonthly
                    ? <Ticket size={20} className={color.iconColor} />
                    : <CircleDollarSign size={20} className={color.iconColor} />}
            </div>
            ...
            <div className={`flex items-center gap-1 px-2.5 py-1 rounded-full ${color.pctBg}`}>
                {isPositive
                    ? <ArrowUpRight size={12} className={color.pctText} />
                    : <ArrowDownLeft size={12} className={color.pctText} />}
